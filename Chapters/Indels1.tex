...
\citep{Green20072x,2007Identification}. The existence of clusters of
...
of positive selection. \tcr{Different aligners may also be designed
...
\end{tabular}
\begin{tablenotes}
\item [] N{\sc ote}.---{\hskip -.1pt}MPL is the mean path length of the tree in units of substitutions per synonymous site. Indel lengths are measured in units of codons, and the indel rate is defined as the number of insertion \& deletion events per substitution.
\end{tablenotes}
\end{threeparttable}
\end{table}

\tcr{In addition, I expect the protein structure and the
  evolutionary divergence of a dataset to contribute to the effects of
  alignment error}. Differently structured protein regions show
...
I focus on sitewise detection of positive selection occurring
...
\defcitealias{Hillier2004Sequence}{}
\defcitealias{Siepel2005Evolutionarily}{}
...
\item [] N{\sc ote}.---{\hskip -.1pt}The root-to-tip $dS$ is
equivalent to the MPL (mean path length) used in our simulations. For
two-species comparisons where the pairwise $dS$ was given, the
root-to-tip $dS$ was calculated as half of the pairwise $dS$ and is
...
\citep{Thompson1994ClustalW}, MAFFT \citep{Katoh2005MAFFT}, \tcr{ProbCons
\citep{Do2005a}, T-Coffee \citep{Notredame2000TCoffee}} and two variants of
...
\tcr{(MPL, defined as the mean root-to-tip branch
...
alignment. \tcrr{(Results for ProbCons, T-Coffee and \pranka
...
\tcrr{(MAFFT and \pranka are omitted from Figure \ref{fig_4}A, B and
  ...
  that of ClustalW and \prankc for all our measures. \pranka slightly
...
\ref{fig_5}. \tcrr{(Filtering results for \pranka, ProbCons and
...
\tcrr{Since the performance measurements shown in Figure \ref{fig_5}
...
In this paper, I investigated the performance of sitewise detection
...
alignment. \tcr{For the detection of positive selection in highly
...
As a result, I cannot unequivocally recommend the use of alignment
...
\tcr{However, such heterogeneity is clearly important to the evolution
  of mammalian proteins \citep{Fay2003}. Many proteins contain combinations of
  structured domains and unstructured regions along their length,
  ...
  evolution. Functional differences between genes may also influence
  the \omg distribution, with some genes or domains showing fewer or
  more neutrally-evolving sites than modeled in our simulations,
  making false positive results either less or more likely,
  respectively. As such, the appropriateness of our simulation scheme
  should be critically considered when evaluating our specific power
  and error rate estimates in the context of real-world data
  analysis.}

\tcr{In the case of a protein evolving with a mix of insertion and
  deletion rates, our results based on uniform rates could be used to
  ...
  higher divergence and indel rates than the rest, then the error
  rates within the less-constrained region should be comparable to our
  ...
  fall somewhere between those observed in our homogeneous simulations
  corresponding to the least difficult and most difficult regions of
  the alignment. Critical to such an analysis is the accurate
  estimation of the local indel rate, for which a number of methods
...
\citep{Ellegren2009Selection}. For example, proteins evolving in \Dr
...
our simulations based on a mammalian-like \omg distribution.
