...
\acrodef{2rgd}[2R]{two rounds of genome duplication}
\acrodef{2r}[2R]{two rounds}
\acrodef{ils}[ILS]{incomplete lineage sorting}
\acrodef{paml}[PAML]{Phylogenetic Analysis by Maximum Likelihood}
\acrodef{encode}[ENCODE]{Encyclopedia of DNA Elements}
\acrodef{optic}[OPTIC]{Orthologous and Paralogous Transcripts in Clades}
\acrodef{mbl2}[\emph{MBL2}]{mannose-binding lectin 2}
\acrodef{masp}[MASP]{MBL-associated serine protease}
\acrodef{tlr}[TLR]{Toll-like receptor}
\acrodef{hcg}[HCG]{human-chimpanzee-gorilla}
\acrodef{hc}[HC]{human-chimpanzee}
\acrodef{gh}[GH]{gorila and human}
\acrodef{gc}[GC]{gorila and chimpanzee}
\acrodef{ch}[CH]{chimpanzee and human}
\acrodef{aga}[AGA]{African great ape}
\acrodef{epo}[EPO]{Enredo-Pecan-Ortheus}
\acrodef{pwf}[PWF]{probability weighting function}
\acrodef{wd}[WD]{working draft}
...
\newcommand{\psgtfive}{TPM$_{0.05}$\xspace}
\newcommand{\psgtten}{TPM$_{0.1}$\xspace}
\newcommand{\psgttwenty}{TPM$_{0.2}$\xspace}
\newcommand{\psgtfifty}{TPM$_{0.5}$\xspace}

\newcommand{\psgfisher}{Fis.\xspace}

\newcommand{\psgeohfive}{Emp$_{0.005}$\xspace}
\newcommand{\psgeone}{Emp$_{0.01}$\xspace}
\newcommand{\psgefive}{Emp$_{0.05}$\xspace}
\newcommand{\psgeten}{Emp$_{0.1}$\xspace}

\newcommand{\wa}{$\bar{\omega}_{A}$\xspace}
\newcommand{\wg}{$\bar{\omega}_{G}$\xspace}

\newcommand{\topgo}{\texttt{topGO}\xspace}
\newcommand{\goseq}{\texttt{goseq}\xspace}
...
\newcommand{\amac}{$\alpha_2$-macroglobulin\xspace}

\newcommand{\pv}{$p$-value\xspace}
\newcommand{\pvs}{$p$-values\xspace}
\newcommand{\lrtmin}{LRT$_{min}$\xspace}
\newcommand{\lrtmax}{LRT$_{max}$\xspace}
